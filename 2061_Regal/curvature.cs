...
/// </summary>
public class Curvature : GH_ScriptInstance
{
    ...
    /// </summary>
    private void RunScript(Curve curves, double min, double max, ref object A)
    {
        List<Point3d> pts = new List<Point3d>();
        //double[] ts = curves.DivideByLength(0.225, true);
        double[] ts = curves.DivideByCount(100, true);
        for (int i = 0; i < ts.Length; i++)
        {
            double radius = curvature(curves, ts[i]);
            if (radius < min) { radius = max; }
            if (radius < max)
            {


                Point3d pt = curves.PointAt(ts[i]);
                pts.Add(pt);
            }
    ...
    double curvature(Curve c, double t)
    {
        double radius = 3.500;
        //double t;
