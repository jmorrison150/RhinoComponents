...
/// </summary>
public class brick_infill : GH_ScriptInstance
{
    ...
    /// </summary>
    private void RunScript(List<Curve> curves, bool reverse, ref object bricks0, ref object bricks1, ref object bricks2, ref object outCourses, ref object outPoints)
    {
        ...
        //for (int i = 150; i < 154; i++)
        {
            ...
            sizeX = 0.2150;
            int currentCourse = 0;
            double currentCourse0 = ((curves[i].PointAtStart.Z - bbox.Min.Z) / (sizeZ + jointSize));

            currentCourse = (int)Math.Round(currentCourse0);
            ...
            {
                //if(intersectionCurves[j].GetLength() < 0.430){continue;}
                ...
                while (currentLength < maxLength)
                {
                    ...
                    double r0 = curvature(intersectionCurves[j], currentLength);
                    double r1 = curvature(intersectionCurves[j], currentLength + 0.1025);
                    double r2 = curvature(intersectionCurves[j], currentLength + 0.215);
                    ...
                    if (currentLength == start && currentCourse % 2 == 0) { offsetX *= 0.5; }
                    ...
                    // if(extraSpace > 1.0){extraSpace = 1.0;}
                ...
                {
                    double rotationCurrent = rotationAngle;
                    Brick brick = new Brick(courseXPoints[k], intersectionCurves[j], currentCourse, k, brickLengths[k]);
                    //brick.setRotation(rotationCurrent);
                    bricks.Add(brick);
                }
                //ends
                Print(lengthLeft.ToString());

                //if(lengthLeft > extraSpace+jointSize){
                if (lengthLeft < 0.0)
                {
        ...
        {
            if (bricks[i].end) { boxes2.Add(bricks[i].draw(reverse)); }
            else if (bricks[i].edge == 0) { boxes0.Add(bricks[i].draw(reverse)); }
            else if (bricks[i].edge == 1) { boxes1.Add(bricks[i].draw(reverse)); }
        }
    ...
    {
        //constructor
        public Brick(Point3d _center, Curve _centerLine, int _courseZ, int _courseX, double _brickLength)
        {
            this.start = _center;
        ...
        public Box draw(bool reverse)
        {
            ...
            intervalY = new Interval(0, -sizeY);
