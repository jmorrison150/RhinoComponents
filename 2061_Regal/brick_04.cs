...
public class Script_Instance : GH_ScriptInstance
{
    ...
    /// </summary>
    private void RunScript(Brep brep, Mesh meshes, List<Curve> porous, Curve rotation, List<Curve> reliefs, Curve zRotation, double reliefDistance, double maxDist, ref object bricks0, ref object bricks1, ref object bricks2, ref object outCourses, ref object outPoints)
    {
        ...
        //for (int i = 150; i < 154; i++)
        {
            ...
            if (!useBrep)
            {
                ...
                try
                {
                    ...
                    {
                        if (pls.Length < 2) { Component.AddRuntimeMessage(GH_RuntimeMessageLevel.Error, "mesh error"); }

                        PolylineCurve plc = new PolylineCurve(pls[j]);
                        Curve c = plc.ToNurbsCurve();
                        double length = c.GetLength(0.001);
                        int cpoints = (int)(length / 0.215);
            ...
            else
            {
                ...
                Rhino.Geometry.Intersect.Intersection.BrepPlane(brep, plane, 0.001, out cvs, out intersectionPoints);
                intersectionCurves.AddRange(cvs);
            }
