...
public class Script_Instance : GH_ScriptInstance {
    ...
    /// </summary>
    private void RunScript(Point3d center_3D, DataTree<Point3d> points_3D, double angle, DataTree<Transform> transforms, ref object angles) {
        ...
        if (points_3D.BranchCount != transforms.BranchCount) { Component.AddRuntimeMessage(GH_RuntimeMessageLevel.Error, "points_3D must match transforms"); return; }
        ...
        Point3d[][] pts = new Point3d[points_3D.BranchCount][];
        for (int i = 0; i < pts.Length; i++) {
            pts[i] = new Point3d[points_3D.Branches[i].Count];
            for (int j = 0; j < pts[i].Length; j++) {
                pts[i][j] = points_3D.Branches[i][j];
            }
        }
        ...
        for (int i = 0; i < pts.Length; i++) {
            for (int j = 0; j < pts[i].Length; j++) {
                Vector3d toCenter = center_3D - pts[i][j];
                for (int k = 0; k < xforms[i].Length; k++) {
                    toCenter.Transform(xforms[i][k]);
                }

                double _angle;
                //_angle = Vector3d.VectorAngle(toCenter, Vector3d.XAxis, Plane.WorldXY);
                _angle = Vector3d.VectorAngle(toCenter, Vector3d.YAxis);
