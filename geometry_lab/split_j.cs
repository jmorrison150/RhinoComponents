...
/// </summary>
public class Script_Instance25 : GH_ScriptInstance {
    ...
    private void RunScript(DataTree<Point3d> points, int index, ref object listA, ref object listB) {
        ...
        for (int i = 0; i < pts0.Length; i++) {
            pts0[i] = new Point3d[index];
            for (int j = 0; j < pts0[i].Length; j++) {
                pts0[i][j] = pts[i][j];
            }
        }
        for (int i = 0; i < pts1.Length; i++) {
            pts1[i] = new Point3d[pts[i].Length-index];
            for (int j = 0; j < pts1[i].Length; j++) {
                pts1[i][j] = pts[i][j+index];
            }
        }
