...
public class Script_Instance4 : GH_ScriptInstance {
    ...
    /// </summary>
    private void RunScript(List<Curve> threads, double unrollWidth, double unrollHeight, ref object outTransforms, ref object reverseForms)
  {
    ...
    for (int i = 0; i < threads.Count; i++) {
      ...
      if (!threads[i].TryGetPlane(out planes[i],0.1)) { 
        Component.AddRuntimeMessage(GH_RuntimeMessageLevel.Error, i.ToString() + " try get plane failed"); continue; }
      ...
      //    pointAtEnd = surfaces[i].PointAt(surfaces[i].Domain(0).Mid, surfaces[i].Domain(1).Mid);
      //}
      ...
      Transform faceFront;
      //= Transform.Rotation(startVector, endVector, pointAtStart);
      Transform faceFront3;
      //= Transform.Rotation(endVector, startVector, pointAtStart);
      //surfaces[i].Transform(faceFront);
      ...
      //surfaces[i].Transform(moveToOrigin);
