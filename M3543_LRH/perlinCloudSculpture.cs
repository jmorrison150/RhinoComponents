...
public class Script_Instance : GH_ScriptInstance {
    ...
    private void RunScript(Brep brep, int seed, int count, double max, double min, ref object A) {
        ...
        for (int i = 0; i < updateBreps.Length; i++) {
            ...
            Interval intervalX = new Interval(sizeX * -0.5, sizeX * 0.5);
            Interval intervalY = new Interval(sizeY * -0.5, sizeY * 0.5);
            Interval intervalZ = new Interval(0, sizeZ);
            Box box = new Box(basePlane, intervalX, intervalY, intervalZ);
            Transform rotate = Transform.Rotation(rotateZ, Vector3d.ZAxis, origin);
            box.Transform(rotate);
            updateBreps[i] = box.ToBrep();
