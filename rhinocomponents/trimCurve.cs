...
public class Script_Instance5 : GH_ScriptInstance {
  ...
  /// </summary>
  private void RunScript(Curve curve, double start, double end, ref object A) {
    double min = curve.Domain.Min;
    double max = curve.Domain.Max;
    double startParam = map(start, 0.0, 1.0, min, max);
    double endParam = map(end, 0.0, 1.0, min, max);

    Interval interval = new Interval(startParam, endParam);
    Curve c =  curve.Trim(interval);
    A = c;
