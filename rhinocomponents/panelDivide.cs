...
public class Script_Instance1 : GH_ScriptInstance {
  ...
  /// </summary>
  private void RunScript(List<Surface> surfaces, double width, ref object outGraph) {
    ...
    List<double> updateGraph = new List<double>();
    ...
    for (int i = 0; i < ss.Length; i++) {
      ...
      Point3d[] pivotPts = pivotLine.DivideEquidistant(width);
      for (int j = 0; j < pivotPts.Length; j++) {
        double param = map(j, 0, pivotPts.Length, 0, 1.0);

       updateGraph.Add(param);

      }
      outGraph = updateGraph;
