...
public class Script_Instance : GH_ScriptInstance {
	...
	/// </summary>
	private void RunScript(Curve arch, Curve profile, List<double> tapers, ref object A) {
		...
		double[] ts = arch.DivideByCount(tapers.Count - 1, true);
		Plane[] planes = new Plane[ts.Length];
		Curve[] profiles = new Curve[ts.Length];

		for(int i = 0; i < ts.Length; i++) {
			arch.FrameAt(ts[i], out planes[i]);
			Plane world = Plane.WorldZX;
			world.Rotate(-90*Math.PI/180.0, Vector3d.ZAxis);
